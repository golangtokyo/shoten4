 ...
 * 引数の値によって戻り値が決まる（内部で状態を持たない）振る舞いであれば、@<code>{func}を利用する。

上記の@<code>{struct}+@<code>{interface}の実装では、@<code>{Hello struct}がフィールドを持たないので、@<code>{DoFunction interface}の@<code>{Exec()}は引数の値によって戻り値が決まる振る舞いになります。
なので、@<code>{func}を利用して処理を差し替える方が適しているでしょう。

しかし、実際はそう簡単に判断できる問題ではありません。将来的な要件によって@<code>{struct}+@<code>{interface}の方が適している場合もあるでしょうし、チームの方針、個人の好みもあるでしょう。使い分けの判断は難しいですが、"なんとなくfuncにする"という個人の感覚による実装は避けましょう。感覚によって実装する癖が付いてしまうと、本来あるべき正しさを考えずにコードを書く癖が付いてしまいます。それを避けるためにも "なぜそういった判断をしたのか?" だけは明確に説明できるようにしておきましょう。空の@<code>{struct}に@<code>{interface}を実装する際は一度立ち止まって考えてみることをおすすめします。
