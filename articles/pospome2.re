...
//list[pospome_list8][funcのメソッドがインターフェースを満たす]{
package main
...
こういった "処理の差し替え" は、インターフェースを利用することでも実現できます。次の例は@<code>{struct}+インターフェースを組み合わせたものです。
//list[pospome_list18][struct+インターフェースの例]{
package main
 ...
 * インターフェースを実装する対象のtypeが機能しないのであれば、関数を利用する。
 * インターフェースを実装する対象のtypeが機能するのであれば、インターフェースを利用する。

@list{pospome_list18}では、@<code>{Hello struct}がフィールドを持ちません。フィールドを持たない@<code>{struct}だからこそ、@<code>{struct}を@<code>{int}に変更しても特に問題が発生することはありません。インターフェースを実装する対象の型は何でもいいのです。つまり、@<code>{struct}は機能していないことになります。@<code>{DoFunction interface}の@<code>{Exec()}は引数の値によって戻り値が決まる振る舞いなので、インターフェースである必要がありません。こういった場合は関数を利用して処理を差し替える方が適しているでしょう。

しかし、実際はそう簡単に判断できる問題ではありません。将来的な要件によってインターフェースの方が適している場合もあるでしょうし、チームの方針、個人の好みもあるでしょう。使い分けの判断は難しいですが、 "なんとなくfuncにする" という個人の感覚による実装は避けましょう。感覚によって実装する癖が付いてしまうと、本来あるべき正しさを考えずにコードを書く癖が付いてしまいます。それを避けるためにも  "なぜそういった判断をしたのか?" だけは明確に説明できるようにしておきましょう。空の@<code>{struct}にインターフェースを実装するコードを書こうとした時は一度立ち止まって考えてみることをおすすめします。
...
これは次のように関数を束ねる@<code>{struct}を用意することで解決できます。
...
複数の関数を@<code>{struct}で定義する場合は、上記の@<code>{Begin},@<code>{Commit},@<code>{Rollback}のように明示的に型を定義すると、
...
一見問題ないように見えますが、コードは時と共に要件が変化していくものです。1年後、@<code>{Do()}の引数に@<code>{struct}を指定したくなる可能性もあります。その際、既存の@<code>{DoFunc}を利用しているコードに影響を与えることは避けたいでしょう。この問題はインターフェースを定義することで解決できます。
...
//list[pospome_list22][引数をfuncからインターフェースに変更]{
package main
...
@<code>{Do()}の引数をインターフェースにすることで、次のように@<code>{struct}を引数に指定することが可能になります。
//list[pospome_list23][インターフェースによってstructを引数に指定する]{
type DoStruct {
	X, Y int
}
...
インターフェースは "複数のtypeに共通する振る舞い" を定義するものです。将来必要になるかも分からない段階でインターフェースを実装してしまうと、実装対象の仕様とコードにミスマッチが生まれてしまい、コードから伝わる意図が間違ったものになってしまいます。意味もなくインターフェースを適用させるのは避けましょう。

反面、不特定多数が利用するライブラリ、フレームワークなどは、利用者のニーズを絞り込むことが難しいことが多いです。利用者によっては、@<code>{struct}に振る舞いを実装したいかもしれませんし、@<code>{int}に振る舞いを実装したいかもしれません。こういった利用者のニーズに幅広く対応するのを目的とした場合、インターフェースを提供する場合があります。
