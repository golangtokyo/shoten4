...
== Women Who Goと私たちのこれまでの活動について
こんにちは。Women Who Go Tokyoの運営スタッフの@akane_256@<fn>{akane_256_fn1}です。
普段は表参道のIT企業で、Webサイトの開発・運用に携わっています。
Women Who Goというコミュニティの歴史と私たちのこれまでの活動についてご紹介します。

//footnote[akane_256_fn1][@<href>{https://twitter.com/akane_256}]

=== Women Who Goとは？
Women Who Goは、2014年にサンフランシスコで、女性・ジェンダーマイノリティの方のためのGoのグローバルコミュニティとして設立されました。12カ国27都市で運営されています。(2018年4月現在)
...
Women Who Go Tokyoは、2016年6月に日本で初めてのWomen Who Goのコミュニティとして設立されました。Goの情報が他の言語と比べて少なかったことと、先述の通り多くの女性やジェンダーマイノリティの方々と一緒に学んでいきたいという想いから、東京支部設立の運びとなり、私は立ち上げのスタッフの一人として参加をしました。
...
不明点などあれば、私たちWomen Who Go Tokyoでもサポート@<fn>{chapter_fn1}いたします。
...
普段のシゴトは、株式会社リブセンスのなんでもやっていくぞマンですが、ちょいちょいGoを織り交ぜていこうと暗躍しています。
 ...
 * Goはコードを静的解析してくれるツールが豊富であるため、ちょっとしたコーディングのミスを防ぐことができる。
...
そして、キャラクターのGopherのかわいらしさも魅力 のひとつです。

=== Goの演習
Go Playgroundを利用することで、すぐにGoを書いて試すことができます。
難しい環境設定は必要ありません。
...
これから、じゃんけんのロジックからテストまでをGo Playgroundで書いていきます。
...
==== 準備をする
じゃんけんの手と結果を数字で計算させようとしているので、それぞれ@<code>{int}型で定義します。
また、じゃんけんロジックは引数を手、戻り値を結果とするメソッドで表現したいため、手を@<code>{Hand}、結果を@<code>{Result}という型を@<code>{type}で定義します。

下記のコードを@<code>{package main}の下に書きます。
...
先ほどのプログラムの一番下にこの関数を追記し、@<code>{main}関数を削除すればGo Playgroundでのテストの完成です。
Runボタンを押下して実行してみると、テストが実行され、@<list>{micchie_list9}のように表示されるはずです。
...
さて、Go Playgroundを利用したGoの演習はここまでとなります。
...
ここでは"Gopher Value(Goコミュニティの目指すべき価値観)"として6つの項目が挙げられています。
  ...
  ** とりわけ、意見の違いを尊重する。
  * 寛大であること
  ** 他の人の意見に真摯に向き合い、粗探しをしない。異議がある場合も、その人の主張を理解しようと努める。
  * 自滅的な行動(下記)を慎むこと
  ...
  ** 無自覚な差別: 素っ気ない陳腐な言葉、態度、雰囲気による侮辱は、個人または団体に対して、敵対的、高圧的、否定的な軽蔑や攻撃の表現になるため、使わない。

私たちWomen Who Go Tokyoも、この"Code of Conduct"に基づいて活動しています。

=== なぜ"女性"コミュニティなのか？
技術の世界はまだまだ男性がマジョリティで、特に新しい言語であるGoの世界では女性の人数が少ないのでは、と感じています。
わたしは昨年たくさんのGoの勉強会やイベントに参加しましたが、その際、広い会場(50人〜100人)でたった1人の女性であることが珍しくありませんでした。それはわたしにとって寂しいと感じる出来事でした。

わたしはこのコミュニティを、女性やジェンダーマイノリティの方が、孤独を感じることなく安心感を得られる場所にしていきたいと思い、運営スタッフとして参画しています。

=== 目指す姿とこれからの活動について
私たちが目指すのは、技術を学ぶモチベーションがある全ての人に対し、ただ彼らが性別的にマイノリティであるというだけでひとりぼっちになるような心理的障壁を打ち壊すこと。
...
またそれを実現するために、2018年は次の2つのアクションをとっていきます。

==== 参加者が安心できる、クリーンであたたかいコミュニティ運営を続けること。
イベントでは女性やジェンダーマイノリティの方々に不快感を与える表現やシチュエーションをつくらないように努めます。
そしてSlackグループでも困ったことや相談ごとがあれば気軽にみんなで話し合える環境を整えたいと思っています。

また、GTUG Girlsのみなさまとの共催イベントで教えてもらった"なごみタイム"(お菓子をたべながらアイスブレイクする時間)は、参加者同士のアイスブレイクにちょうどよかったので、Women Who Go Tokyoでも"Gopherカフェ"として取り入れたいと思っています。

==== 参加者が教えてもらうだけではなく、参加者やスタッフ同士で教え合える機会をつくること。
...
ハンズオン形式やものづくりイベントの際も、私たちスタッフがみんなでサポートしますので、初心者の方もどうぞ安心してご参加ください。
また、質問やわからないことがあればSlackでも気軽に質問し合っていますので、ぜひ一度参加してみてください。

=== "女性限定"のコミュニティであるというイメージを払拭すること。
私たちはGoを学ぶモチベーションがあるすべての人に対し、オープンなコミュニティでありたいと思っています。
私たちが目指すのは、性別上マイノリティであるというだけで、イベントやコミュニティに参加する際に不安や落ち着かなさを感じるような状況をなくすことです。男性を排除することは私たちの目的ではありません。

のイベントの参加者については女性とジェンダーマイノリティに限定していますが、講師やチューター、運営のボランティアとして性別に関わらず多くの方々にご支援、ご賛同いただき活動してきました。
私たちは、私たちの考えに賛同していただけるみなさんとともにこれからも活動していきたいと強く願っています。

また、私たちはHumans Who Go Tokyoというもくもく会を開催しています。これは普段のイベントとは異なり、性別の隔てなく全ての人に向けて公開したイベントです。
第1回の参加者はスタッフが中心でしたが、有志のGopherが集まってもくもくする楽しいイベントとなりました。
このようなイベントを、形式を模索しながら今後も続けていくつもりです。
...
ぜひ遊びに来てください。私たちはGopher仲間を待っています。
