...
## 1. これまでの活動について
こんにちは。Women Who Go Tokyoの運営スタッフの@akane_256です。  
...
Women Who Goとして実施する活動の内容は、各国で自由で、ベルリンでは「Go In Action」の 著者を招いてワークショップを行ったり、シカゴではGoの新バージョンがリリースされた際にリリースパーティを行ったりしています。そして、そのイベントの多くは無料で開催されています。

他国の活動については、＠womenwhogoのTwitterアカウントから見ることができます。
...
その際のスライドについても、こちらのURLからご覧いただけます。
https://speakerdeck.com/mom0tomo/gotu-ran-ri-ji

#### プログラミング初学者向けのワークショップ
...
下記は、sliceと繰り返し文の練習で実際に書いたコードです。
...
Women Who Go Tokyoのその他の活動については、@wwg_tokyoのTwitterアカウントをのぞいてみて下さい。もし少しでもご興味があれば、ぜひ気軽に足を運んでいただけるととても嬉しいです。

### 東京以外の活動について
現在日本にあるWomen Who Goコミュニティは東京だけですが、東京以外でも支部を新設することができます。下記サイトのルールをもとに、Women Who Goまでご連絡ください。  
...
- gofmt, golint, goimports, go vetといった標準のツールがあり、Goの正解に自然と導いてくれる。
...
### Goの演習
`Playground` を利用することで、すぐにGoを書いて試すことができます。  
難しい環境設定は必要ありません。
...
ここで、import全体と、mainの中にかいてある `fmt.Println("Hello, playground")` を消してしまいます。  
スッキリしてしまいました。
...
これから、じゃんけんのロジックからテストまでを `Playground` で書いていきます。
...
と数字で見立てると、下記のような計算式で勝ち負けを判別することができます。(`%` は余りです)
...
よって下記のコードを `package main` の下に書きます。
...
ここで出てくる `iota` は、定数で利用される、型なしの連続する整数定数で、最初は `0` で初期化され、1つずつインクリメントされます。2行目移行の `iota` は省略が可能です。
...
#### 4. main関数でじゃんけんを実行する
`you` と `challenger` にそれぞれ好きな手を入れてみてください。  
...
`Run` ボタンを押下して実行してみると、下記のように表示されるはずです。
...
これは、先程 `const` で定義しているので、`you: Rock, challenger: Scissors, Result: you Win...` と同義です。  
この演習では、そのまま数字で出力されてしまいますが、文字列に置き換えて表示させることも可能です。  
  
`Playground` は、`Share` ボタンを押下することでソースコードのURLが発行でき、簡単に他の人に共有することができます。  
この演習プログラムのURLは、`https://play.golang.org/p/zyXc7WsyQuC` です。

#### 5. テストを書く
playgroundはテストを書くこともできます。  
ポイントは2つです。

- main関数がないこと。
- `func Test{関数名}(t *testing.T)` を用意すること。
...
先ほどのプログラムの一番下にこの関数を追記し、main関数を削除すれば `Playground` でのテストの完成です。  
`Run` ボタンを押下して実行してみると、テストが実行され、下記のように表示されるはずです。  
...
このテストプログラムのURLは、`https://play.golang.org/p/NvHvqUvb2xV` です。  
さて、`Playground` を利用したGoの演習はここまでとなります。

## 3. わたしたちの目指す姿と今後の展望
Women Who Go Tokyoの[@mom0tomo](https://twitter.com/mom0tomo)です。  
...
女性が技術コミュニティにおいてマイノリティであるのは、現在の社会情勢を見れば、あと十数年間は我慢しなければならない状況でしょう(大学の情報工学科の女性比率は何％でしょう？ちなみにわたしの卒業した大学では、2017年度入学者1,052名のうち女性は82名で10%未満でした)。  
  
わたしたちのコミュニティは、
- 女性あるいはジェンダーマイノリティであり
- Goに興味を持ち学ぼうとしている
 みなさんに向けてドアを開き、安心してブログラミングや創作活動ができる場所を提供することを目指します。  
  
またそれを実現するために、以下の3つを2018年のアクションとして掲げます。

#### 1."女性のみ"のコミュニティであるというイメージを払拭すること。
わたしたちはGoを学ぶモチベーションがある全ての人に対し、オープンなコミュニティでありたいと思っています。  
わたしたちの目標は、性別上マイノリティであるだけで、イベントやコミュニティに参加することについて、不安や落ち着かなさを感じる機会をなくすことです。  
  
具体的には、いままでイベントページ等で"女性のみ"と記載してきた箇所を修正し、ジェンダーマイノリティにもオープンなコミュニティであるということを全面に打ち出していきます。

#### 2. 参加者が安心できる、クリーンであたたかいコミュニティ運営を続けること。
イベントでは女性やジェンダーマイノリティの方々に不快感を与える表現やシチュエーションをつくらないように努めます。
Slackグループでも問題があったら報告できるチャンネルを作り、気軽に相談したり話をできる環境を整えたいと思っています。  
  
また、先日GTUG Girlsのみなさまとの共催イベントで教えてもらった「なごみタイム」(お菓子をたべながらアイスブレイクする時間)は、参加者どうしで緊張をほぐすのにちょうどいい時間でした。  
今後Women Who Goでも「なごみタイム」を取り入れたいと思っています。

#### 3. 参加者が教えてもらうだけではなく、参加者同士で教え合える機会をつくること。
...
### 私たちの理念に賛同していただける方へ
Women Who Go Tokyoは尊敬すべきエンジニアの方々に支えていただき、活動を続けてきました。  
また活動に賛同していただいた企業のみなさまにご協力いただき、会場をお貸しいただいてきました。  
これまでご支援いただいたみなさまに改めて感謝します。  
そして、これからもよろしくお願いいたします。  

## さいごに
Women Who Go Tokyoは、もっと多くの方々と一緒にGoを学んでいきたいと考えております。Goをさわってみたい方ならどなたでも大歓迎です！  
Macだけではなく、Windows、Ubuntu環境も用意して、環境構築で躓いたときにはいつでもサポートできるようにしています。  
ぜひ遊びに来てください。わたしたちはGopher仲間を待っています。  
そして、一緒に活動を盛り上げてくださる方を募集しています。もし少しでもご興味がある方は、ぜひ@wwg_tokyoのTwitterアカウントまでご連絡ください。
